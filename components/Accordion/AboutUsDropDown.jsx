...
import { approveIcon } from "@public";
  ...
  return (
    <div
      className={`mb-3 cursor-pointer select-none rounded-xl p-4 shadow-md lg:p-6 ${
        isOpen ? "bg-yellow-300/30" : "bg-slate-800"
      }`}
      onClick={toggleFAQ}
    >
      <div className="flex w-full flex-col items-start justify-between lg:flex-row lg:items-center">
        <div className="mt-4 lg:w-3/4">
          <h2 className="mb-3 text-2xl font-extrabold text-primary-one">
            {heading}
          </h2>
          <div className="flex items-start font-semibold text-primary-one">
            {/* Use flex for icon and text */}
            <Image src={approveIcon} alt="Approve Icon" className="mr-2 " />
      ...
      <ul className={isOpen ? " font-semibold text-primary-one" : "hidden"}>
