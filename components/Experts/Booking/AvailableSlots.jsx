...
import ChooseMonthBtn from "@components/Buttons/ChooseMonthBtn";
  ...
  return (
    <div className="my-10 gap-2 lg:flex lg:gap-8">
          ...
          {slots[index]?.slots.map((value, i) => {
            return (
              <button
                ...
                key={i}
                className={`cursor-pointer rounded-lg border-2 border-primary-one p-1 py-2 text-center text-sm font-semibold  transition-all ${
                  slotIndex == slots[index]?.slots.indexOf(value)
                    ? "bg-primary-one"
                    : "text-primary-one hover:bg-primary-one hover:text-black"
                }`}
              >
                {value.date}
              </button>
      ...
      <div className="lg:w-1/2">
        <h2 className="mb-6 text-center text-lg font-bold text-slate-400">
          Select time slots
        </h2>
        ...
        <div className="grid grid-cols-4 gap-2">
          {slots[index]?.slots[slotIndex].time.map((value, i) => (
            <button
              ...
              className={`block cursor-pointer rounded-full border-2 border-primary-one py-2 text-center text-sm font-medium transition-all  ${
