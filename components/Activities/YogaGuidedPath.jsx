...
export default function YogaGuidedPath({ title }) {
  ...
  return (
    <section className="pt-10 p-2 pb-6 xl:pb-10 bg-slate-900">
      <div className="container mx-auto">
        <div className="mb-8">
          ...
          <div className="flex justify-center">
            <span className="inline-block h-[3px] w-8 rounded-full bg-primary-one"></span>
          </div>
        </div>
        {/* weeks */}
        <div className="mb-10 flex justify-center gap-10">
          <p
            className={`text-center font-bold sm:text-2xl text-slate-200 ${
          ...
          {selectedWeek === "week1" &&
            CURRICULUM_DATA?.slice(0, 5)?.map((el, index) => (
              <>
                <div
                  key={index}
                  className="col-span-1 flex items-center justify-center rounded-lg bg-yellow-500 p-2 text-lg font-semibold xs:block xs:p-5 xs:text-xl"
                >
                  <p className="">{el?.day}</p>
                </div>
                <div className="col-span-2 rounded-lg bg-yellow-400 p-2 text-lg font-semibold xs:p-5 xs:text-xl">
                  <p>{el?.yoga}</p>
                </div>
              </>
            ))}
